    ...
    'lessons': [
        {
            ...
            'concept': 'hashcat is how you guess passwords against hashes at GPU speed, by generating candidates, hashing them, and comparing. That is why it is the tool for a large hash set or a slow hash: the hardware is the rate, and every flag is either a faster hash or a better guess. Cracking is not reversing a hash.\n\nTwo numbers run the whole tool and confusing them is the classic first error. `-m` is the hash mode: what kind of hash you have, where 0 is MD5, 1000 is NTLM, 3200 is bcrypt, 1800 is SHA-512-crypt. `-a` is the attack mode: how candidates are generated, where 0 is a straight wordlist, 3 is a brute-force mask, and 6 and 7 are hybrids. Get `-m` from what the hash is and `-a` from your strategy.\n\nWhat the swap looks like: `-m 1000` is NTLM. `-a 1000` is not an attack, and hashcat never starts. The other miss is treating it as a decoder: a "decrypt" site returns a password because someone already guessed that hash, not because the function was inverted. Against a salted hash the same site finds nothing: each salt makes the lookup table worthless.\n\nScope, as everywhere in this group: how the tool works, never who to point it at. Crack hashes you generated or are permitted to crack, and nothing else.',
            'examples': [
                {
                ...
                {
                    'label': 'The classic first swap',
                    'code': 'hashcat -m 1000 -a 0 hashes.txt rockyou.txt\n        NTLM, wordlist. correct.\n\nhashcat -m 0 -a 1000 hashes.txt rockyou.txt\n        MD5, attack 1000. there is no attack 1000.',
                    'note': '-m 1000 is NTLM. -a 1000 is not an attack. Mixing the two numbers is the error this lesson exists to prevent.',
                },
            ],
            'misconceptions': [
        ...
        },
        {
            ...
            'examples': [
                ...
                {
                    'label': 'Name the run, resume it, write a report',
                    'code': 'hashcat -m 1000 --session=ntlm hashes.txt rockyou.txt\nhashcat --session=ntlm --restore\n\nhashcat -m 1000 --username users.txt -o cracked.txt rockyou.txt\n\nusers.txt is user:hash; --username skips the name\n-o is the report; --show still reads the potfile',
                    'note': 'Ctrl-C then --restore continues the same session. -o does not replace the potfile; it is a second copy you can keep with the job.',
                },
            ],
            'misconceptions': [
